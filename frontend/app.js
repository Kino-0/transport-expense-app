...
class ApiService {
  ...
  async fetchHistory(empCode) {
    const { data: historyList, error } = await this.supabase.rpc(
      "get_expense_history",
      { p_emp_code: empCode },
    );
    if (error)
      throw new Error(`申請履歴の取得に失敗しました: ${error.message}`);
    return historyList;
  }
  ...
  async fetchDetails(applId) {
    const { data, error } = await this.supabase
      ...
      .maybeSingle();
    if (error)
      throw new Error(`申請詳細の取得に失敗しました: ${error.message}`);
    if (!data) throw new Error(`申請詳細が見つかりません: ${applId}`);
    return data;
  }

  async submitApplication(empCode, detailsData) {
    const { data: newApplId, error } = await this.supabase.rpc(
...
class UIManager {
  ...
  updateLineTotal(tr) {
    const unitPrice =
      parseInt(tr.querySelector('[data-role="unitPrice"]').value, 10) || 0;
  ...
  renderHistory(historyData) {
    ...
    if (!historyData || historyData.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="5" class="p-4 text-center text-gray-500">申請履歴はありません。</td></tr>';
      return;
    }
  ...
  showDetailsModal(data) {
    ...
    this.dom.modalTotalAmount.textContent =
      (data.total_amount || 0).toLocaleString() + " 円";
    ...
    if (!data.details || data.details.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="8" class="p-4 text-center text-gray-500">明細はありません。</td></tr>';
    } else {
      tbody.innerHTML = data.details
        .map(
          (item) => `
                <tr>
...
class AppController {
  ...
  bindEvents() {
    ...
    this.ui.dom.logoutButton.addEventListener("click", () =>
      this.handleLogout(),
    );
    this.ui.dom.navContainer.addEventListener("click", (e) =>
      this.handleNavClick(e),
    );
    this.ui.dom.addRowButton.addEventListener("click", () =>
      this.ui.createRow(),
    );
    this.ui.dom.submitButton.addEventListener("click", () =>
      this.handleSubmit(),
    );
    ...
    this.ui.dom.historyTableBody.addEventListener("click", (e) =>
    ...
    [
      ...
      this.ui.dom.modalCloseButtonFooter,
    ].forEach((el) => {
  ...
  handleTableEvent(e) {
    ...
    if (
      (role === "unitPrice" && e.type === "input") ||
      (role === "isRoundTrip" && e.type === "change")
    ) {
  ...
  async handleSubmit() {
    ...
    try {
      await this.api.submitApplication(
        this.state.currentUser.empCode,
        detailsData,
      );
      this.ui.showMessage(
        this.ui.dom.submitMessage,
        "申請が完了しました。",
        false,
      );
  ...
  async loadHistory() {
    ...
    try {
      const historyData = await this.api.fetchHistory(
        this.state.currentUser.empCode,
      );
  ...
  collectAndValidateDetails() {
    ...
    const getVal = (tr, role) =>
      tr.querySelector(`[data-role="${role}"]`)?.value.trim() ?? "";
    const getChecked = (tr, role) =>
      tr.querySelector(`[data-role="${role}"]`)?.checked ?? false;
    const getNum = (tr, role) =>
      parseInt(tr.querySelector(`[data-role="${role}"]`)?.value, 10) || 0;
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      if (rowErrors.length > 0) {
        ...
        // バリデーション通過
        detailsData.push({
          use_date: values.useDate,
          purpose: values.purpose,
          line_name: values.lineName,
          dep_station: values.departure,
          arr_station: values.arrival,
          unit_price: values.unitPrice,
          is_round_trip: values.isRoundTrip,
        });
      }
    }
